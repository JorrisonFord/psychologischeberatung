export const de = {
  nav: {
    about: "Über mich",
    services: "Angebot",
    philosophy: "Arbeitsweise",
    testimonials: "Erfahrungen",
    contact: "Kontakt",
  },

  hero: {
    subtitle: "ACT-basierte psychologische Beratung",
    title: "Ein Raum für Klarheit, Ruhe und innere Ausrichtung",
    description:
      "In meiner psychologischen Beratung unterstütze ich Sie dabei, einen bewussteren Umgang mit Gedanken und Gefühlen zu entwickeln und ein Leben zu gestalten, das sich an Ihren persönlichen Werten orientiert. Im Mittelpunkt steht nicht Veränderungsdruck, sondern die Entwicklung psychologischer Flexibilität und mehr Raum für das, was Ihnen wirklich wichtig ist.",
    ctaPrimary: "Termin vereinbaren",
    ctaSecondary: "Mehr erfahren",
  },

  about: {
    subtitle: "Über mich",
    title: "Psychologische Beratung für persönliche Entwicklung und Klarheit",
    paragraph1:
      "Ich bin Master-Psychologe mit Schwerpunkt Neuropsychologie und arbeite integrativ mit psychologischer Beratung, Achtsamkeit und Acceptance & Commitment Therapy (ACT). Im Mittelpunkt steht die Förderung psychologischer Flexibilität und ein bewusster Umgang mit inneren Prozessen.",

    paragraph2:
      "Ergänzend bringe ich langjährige Erfahrung in Yoga und Körperarbeit sowie eine erlebnispädagogische Ausbildung in meine Arbeit ein. Dadurch entsteht ein ganzheitlicher Zugang, der Körper, Emotion und Denken miteinander verbindet.",

    paragraph3:
      "Mir ist es wichtig, Ihnen in einem ruhigen und verlässlichen Rahmen zu begegnen, in dem echte Offenheit entstehen kann.",

    credentials: {
      title: "Qualifikationen",
      items: [
        "M.Sc. Psychologie (Schwerpunkt Neuropsychologie)",
        "Weiterbildung in Acceptance & Commitment Therapy (ACT)",
        "Erfahrener Yoga- und Achtsamkeitslehrer",
        "Ausbildung in Erlebnispädagogik",
      ],
    },
  },

  services: {
    subtitle: "Angebot",
    title: "Meine Schwerpunkte",
    items: {
      counseling: {
        title: "Psychologische Beratung",
        description:
          "In einem sicheren und vertraulichen Rahmen begleite ich Sie bei persönlichen Krisen, Ängsten und herausfordernden Lebenssituationen. Gemeinsam entwickeln wir neue Perspektiven und konkrete Handlungsmöglichkeiten.",
        features: ["Vertraulicher Rahmen", "Individuelle Beratung", "Klarheit & Orientierung"],
      },

      yoga: {
        title: "Achtsamkeit & Körperarbeit",
        description:
          "Durch Achtsamkeit, Atemarbeit und sanfte Körperübungen stärken Sie die Fähigkeit, innere Prozesse bewusster wahrzunehmen und Stress besser zu regulieren.",
        features: ["Achtsamkeitspraxis", "Atemarbeit", "Emotionsregulation"],
      },

      act: {
        title: "ACT & Wertearbeit",
        description:
          "Acceptance and Commitment Therapy (ACT) unterstützt Sie dabei, Gedanken und Gefühle anzunehmen, ohne von ihnen gesteuert zu werden, und ein Leben zu gestalten, das sich an Ihren persönlichen Werten orientiert.",
        features: ["Akzeptanz statt Vermeidung", "Werteorientierung", "Psychologische Flexibilität"],
      },
    },
  },

  philosophy: {
    subtitle: "Arbeitsweise",
    title: "Ein ACT-basierter und achtsamer Ansatz in der psychologischen Beratung",
    pillars: {
      empathy: {
        title: "Empathie",
        description:
          "Ich begegne Ihnen mit Offenheit und echtem Interesse für Ihre individuelle Lebenssituation. In einem Raum von Akzeptanz darf alles da sein, was Sie mitbringen.",
      },
      safety: {
        title: "Sicherheit",
        description:
          "Vertraulichkeit und ein geschützter Rahmen bilden die Grundlage unserer Zusammenarbeit. Hier können Sie sich öffnen, ohne bewertet zu werden.",
      },
      wholeness: {
        title: "Ganzheitlichkeit",
        description:
          "Körper, Gedanken und Emotionen stehen in ständiger Wechselwirkung und prägen unser Erleben. Mein Ansatz integriert diese Ebenen zu einem stimmigen Gesamtbild.",
      },
    },
  },

  testimonials: {
    subtitle: "Erfahrungen",
    title: "Was sich durch die Beratung verändert hat",
    items: [
      {
        quote:
          "Ich habe gelernt, meine Gedanken nicht mehr sofort als Wahrheit zu sehen. Dadurch kann ich ruhiger reagieren und bewusster entscheiden.",
        author: "Klientin, 30",
        context: "ACT & Achtsamkeit",
      },
      {
        quote:
          "Ich habe aufgehört, ständig gegen meine innere Unruhe anzukämpfen. Das hat deutlich Druck aus meinem Alltag genommen.",
        author: "Klient, 40",
        context: "Psychologische Beratung",
      },
      {
        quote:
          "Die Kombination aus Gesprächen und Körperarbeit hilft mir, mich wieder mehr zu spüren und klarer zu erkennen, was mir wichtig ist.",
        author: "Klientin, 28",
        context: "Achtsamkeit & Wertearbeit",
      },
    ],
  },

  contact: {
    subtitle: "Kontakt",
    title: "Nehmen Sie Kontakt auf",
    description:
      "Ich freue mich darauf, Sie kennenzulernen. Vereinbaren Sie gerne ein unverbindliches Erstgespräch.",

    form: {
      name: "Ihr Name",
      email: "E-Mail-Adresse",
      phone: "Telefon (optional)",
      message: "Ihre Nachricht",
      submit: "Nachricht senden",
      success: "Vielen Dank! Ich melde mich bei Ihnen.",
    },

    info: {
      address: "Praxisadresse",
      phone: "Telefon",
      email: "E-Mail",
      hours: "Sprechzeiten",
    },

    details: {
      address: "Schildergasse 69-73\n50667 Köln",
      phone: "+49 1578 879 4109",
      email: "joris.vanbohemen@gmail.com",
      hours: "Mo–Fr: 9:00 – 18:00 Uhr",
    },
  },

  footer: {
    copyright: "© 2026 Psychologische Beratung. Alle Rechte vorbehalten.",
    privacy: "Datenschutz",
    imprint: "Impressum",
  },

  language: {
    de: "DE",
    en: "EN",
  },
};